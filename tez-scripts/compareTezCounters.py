...
def plotGraphVertex(vertexResults):
    fig, axs = plt.subplots(2, 2, figsize=(10,10))
    n = len(vertexResults)
    
    vertices_len = max([len(vertexResults[i]) for i in range(n)])
    ...
    y_axis_num_tasks_map = []
    y_axis_num_tasks_reduce = []
...
#        dag_len = len(filteredDagResults[i])
        ...
        y_axis_num_tasks_map.append([])
        y_axis_num_tasks_reduce.append([])
        
        Map = 0
        Reduce = 0

        for j in range(len(vertexResults[i])):
            if "Map" in vertexResults[i][j][VertexProperties.index('vertexName')]:
                Map += 1
                y_axis_cpu_util_map[i].append(vertexResults[i][j][VertexProperties.index('avgTaskCPUutil')])
                y_axis_spillage_map[i].append(vertexResults[i][j][VertexProperties.index('avgTaskSpilledRecordsPerSec')])
                y_axis_num_tasks_map[i].append(vertexResults[i][j][VertexProperties.index('numSucceededTasks')])

            if "Reduce" in vertexResults[i][j][VertexProperties.index('vertexName')]:
                Reduce += 1
                y_axis_cpu_util_reduce[i].append(vertexResults[i][j][VertexProperties.index('avgTaskCPUutil')])

                y_axis_spillage_reduce[i].append(vertexResults[i][j][VertexProperties.index('avgTaskSpilledRecordsPerSec')])
                y_axis_num_tasks_reduce[i].append(vertexResults[i][j][VertexProperties.index('numSucceededTasks')])
    ...
    for i in range(n):
        ...
        x_axis = [i for i in range(len(y_axis_num_tasks_map[i]))]
        axs[0].plot(x_axis, y_axis_num_tasks_map[i], label=(str(label)+" GB"))
        
        x_axis = [i for i in range(len(y_axis_num_tasks_reduce[i]))]
        axs[1].plot(x_axis, y_axis_num_tasks_reduce[i], label=(str(label)+" GB"))

        label *= 2
        
    axs[0].set_ylabel('Num Tasks/Vertex - Map')
    axs[0].set_xlabel('Vertices')
    axs[0].legend(loc="upper left")

    axs[1].set_ylabel('Num Tasks/Vertex - Reduce')
...
def compareVertices(vertexExpResults, dagResult):
    ...
    results = [['dagId'] + ['vertexId'] + ['vertexName'] + ['CPU_util'] * num_exps + ['CPU_monotonicity'] + ['CPU_CoV'] + ['Spillage'] * num_exps + ['Spillage_monotonicity'] + ['Spillage_CoV']]

    for i in range(num_dags):
        dagId  = [dagResult[i][DagProperties.index('dagId')]]
        dag = [dagResult[i][DagProperties.index('dagDesc')]]
            ...
            for j in range(1, num_exps):
                ...
                for k in range(len(data[j][idx])):
                    if name == data[j][idx][k][VertexProperties.index('vertexName')]:
                        ...
                        cpu_CoV = variation(cpu_utils)
                        spillage_CoV = variation(spillage)
...
#                    continue
                ...
                cpu_utils.append(cpu_CoV)
                spillage_prop = get_property(spillage)
#                if spillage_prop is None:
#                    continue
                spillage.append(spillage_prop)
                spillage.append(spillage_CoV)
                result = dagId + dag + [vertex[VertexProperties.index('vertexId')]] + [name] + cpu_utils + spillage 
